...
def openauctions(request,):
    all_auctions = Auctions.objects.all()
    return render(request, 'auctions/openauctions.html',{
        'all_auctions':all_auctions
    })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
                ...
                if last_price is not None:
                    if last_price.price < form_save.price:
                        auctions.bid_price = form_save.price
                        form_save.save()
                        auctions.save()
                        return HttpResponseRedirect(reverse('auction' , args=(auctions.Auction_title,)))
                    else:
                ...
                elif auctions.Auction_price < form_save.price:
                    auctions.bid_price = form_save.price
                    form_save.save()
                    auctions.save()
                    return HttpResponseRedirect(reverse('auction' , args=(auctions.Auction_title,)))
